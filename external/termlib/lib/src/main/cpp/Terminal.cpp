...
bool Terminal::dispatchMouseWheel(int row, int col, bool scrollUp) {
    std::lock_guard<std::recursive_mutex> lock(mLock);
    ...
    row = std::max(0, std::min(row, mRows - 1));
    col = std::max(0, std::min(col, mCols - 1));

    vterm_mouse_move(mVt, row, col, VTERM_MOD_NONE);
    vterm_mouse_button(mVt, scrollUp ? 4 : 5, true, VTERM_MOD_NONE);
    return true;
}
...
Java_org_connectbot_terminal_TerminalNative_nativeDispatchMouseWheel(JNIEnv* /* env */, jobject /* thiz */,
                                                                     jlong ptr, jint row, jint col, jboolean scrollUp) {
    auto* term = reinterpret_cast<Terminal*>(ptr);
    return term->dispatchMouseWheel(row, col, scrollUp == JNI_TRUE);
}
