...
internal class TerminalNative(callbacks: TerminalCallbacks) : AutoCloseable {
     ...
     *
     * libvterm emits the active mouse protocol selected by the remote app
     * (X10, SGR 1006, rxvt, etc.). Wheel up/down map to buttons 4/5.
     */
    fun dispatchMouseWheel(row: Int, col: Int, scrollUp: Boolean): Boolean {
        checkNotClosed()
        return nativeDispatchMouseWheel(nativePtr, row, col, scrollUp)
    }
    ...
    private external fun nativeDispatchMouseWheel(ptr: Long, row: Int, col: Int, scrollUp: Boolean): Boolean
