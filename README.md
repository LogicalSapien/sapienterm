...
```sh
git clone https://github.com/LogicalSapien/sapienterm.git
cd sapienterm
...
| Command | Description |
|---|---|
| `assembleOssDebug` | Open-source flavor, debug (default for local dev) |
| `assembleOssRelease` | Open-source flavor, release (requires signing config) |
| `assembleGoogleDebug` | Google Play flavor, debug |
| `assembleGoogleRelease` | Google Play flavor, release (requires signing config) |
...
SapienTerm does not collect, transmit, or share any personal data. All SSH credentials and connection settings are stored locally on your device using Android Keystore encryption. See [PRIVACY.md](PRIVACY.md) for the full privacy policy.
