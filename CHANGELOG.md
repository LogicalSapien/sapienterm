...
## [Unreleased]
### Added
- Complete rewrite of the UI using Jetpack Compose with a full design-token theme system (SapienTheme)
- Three-theme system: NeoTerminal (dark), SoftPro (light), QuietLuxury (light)
- Per-theme density tokens (Compact / Comfortable / Spacious)
- Home screen with segmented Hosts / Quick Commands / Keys tabs and persistent filter
- Global search screen: hosts, commands, credentials, quick-create host, show-more expand
- Search now sends commands directly to the active terminal session
- Search credential tap navigates to the Credentials vault
- Credentials vault for storing passwords and SSH key pairs (AES-256-GCM encrypted via Android Keystore)
- Quick Commands screen with categories, swipe-to-delete, and send-to-session
- Settings hub with category cards and search
- Three-tab bottom navigation (Home / Search / Settings)
- Pill-style session tabs in console with per-host accent colours and connection-state dots
- Ambient glow effect in terminal console derived from host accent colour
- Transcript reader with tokenized colours
- Custom bottom bar editor: design your own shortcut strip, globally or per host
- Bottom bar presets: Standard, Zellij (^S/^D), tmux (^B/^D); per-host override
- Host connection groups with create/rename/delete and move-to-group
- Host accent colour categories (16 colours) visible in list and connection cards
- Biometric gate on app foreground: re-triggers every time the app returns from background
- Hardware-keyboard selection mode: DPAD arrows move selection, DPAD Center and Ctrl+Shift+C copy to clipboard via SelectionController
...
- Downloadable fonts and local font import support (by nindanaoto)
...
- All screens consistently use SapienTheme tokens; no direct MaterialTheme.colorScheme calls remain in app UI screens
- Connection lost overlay now dismisses immediately on reconnect and re-arms correctly on subsequent failures
- Settings strings rewritten in plain English (two passes); removed jargon, fixed unclosed string literals, shortened verbose summaries
- Fixed command send not triggering execution in tmux: all command-inject paths now send CR (`\r`) instead of LF (`\n`) to match hardware Enter key
- Transcript reader rebuilt: replaced polling LazyColumn with the live `Terminal` composable — identical pixel-perfect rendering to the main console, including Claude CLI diffs, TUI boxes, and all ANSI colours; no more lag, polling artefacts, or custom text rendering
- Transcript reader: send from the compose field now reaches the terminal directly (same `sendTranscriptReaderInput` path, bracketed-paste toggle preserved)
- Terminal canvas background wired to colour-scheme default background via `terminalColorsFlow`; eliminates per-cell background rects in light themes that caused horizontal stripe artefacts
