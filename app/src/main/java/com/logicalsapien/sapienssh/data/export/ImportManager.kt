...
/**
 * Manages importing SapienSSH backup data from `.sapienssh` (encrypted) or
 * `.json` (plain) files.
 *
...
class ImportManager(
     ...
     * @param passphrase Passphrase for decrypting `.sapienssh` files (null for plain JSON)
     ...
     * @param passphrase Passphrase for decrypting `.sapienssh` files
    ...
    private fun isEncryptedFile(uri: Uri): Boolean {
        ...
        return path.endsWith(".sapienssh", ignoreCase = true)
    }
}
