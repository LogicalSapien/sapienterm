...
object CliPromptDetector {
    ...
    // Password / passphrase prompts (case-insensitive)
    private val PASSWORD_PATTERN = Regex(
        """(?i)(?:password|passphrase|passcode)\s*(?:for\s+\S+\s*)?[:\s]*$|\[sudo\]\s*password"""
    )

    // AI coding tool prompts (inquirer.js, Claude, Copilot, etc.)
    // Matches lines starting with ? or ❯ followed by a question, or lines ending with ›
    private val AI_QUESTION_PATTERN = Regex(
        """^\s*[?❯❮\u276F]\s+.{5,}[?:]\s*$|^\s*[?❯❮\u276F]\s+.{5,}\s*$"""
    )

    // Generic question ending with colon or question mark (conservative: requires 10+ chars)
    private val GENERIC_QUESTION_PATTERN = Regex(
        """^.{10,}[?:]\s*$"""
    )

    // Lines that look like a shell prompt (to EXCLUDE from generic question matching)
    private val SHELL_PROMPT_PATTERN = Regex(
        """[$#%>]\s*$|^\s*\S+@\S+[:\s]|^\s*\w+\s*[#$>%]\s*$"""
    )
    ...
    fun detect(recentLines: List<String>): DetectedPrompt? {
        ...
        // 5. Password / passphrase prompts
        detectPassword(lastLine)?.let { return it }

        // 6. AI tool / generic question prompts (least specific, checked last)
        detectQuestion(lastLine)?.let { return it }
    ...
    private fun detectPassword(text: String): DetectedPrompt? {
        if (!PASSWORD_PATTERN.containsMatchIn(text)) return null
        return DetectedPrompt(
    ...
    private fun detectQuestion(text: String): DetectedPrompt? {
        // AI coding tool prompt (inquirer.js style "? What would you like?" etc.)
        if (AI_QUESTION_PATTERN.containsMatchIn(text)) {
            return DetectedPrompt(
                type = PromptType.QUESTION,
                options = emptyList()
            )
        }
        // Generic question: line ending with ? or : but NOT a shell prompt
        if (GENERIC_QUESTION_PATTERN.containsMatchIn(text) &&
            !SHELL_PROMPT_PATTERN.containsMatchIn(text)
        ) {
            return DetectedPrompt(
                type = PromptType.QUESTION,
                options = emptyList()
