...
package com.logicalsapien.sapienterm.transport
...
import com.logicalsapien.sapienterm.data.entity.KeyStorageType
...
import com.logicalsapien.sapienterm.service.requestBiometricAuth
import com.logicalsapien.sapienterm.service.requestBooleanPrompt
import com.logicalsapien.sapienterm.service.requestHostKeyFingerprintPrompt
import com.logicalsapien.sapienterm.service.requestStringPrompt
import com.logicalsapien.sapienterm.util.HostConstants
import com.logicalsapien.sapienterm.util.PubkeyUtils
...
class SSH :
    ...
    private var credentialFailed = false
    private var lastInteractivePassword: String? = null
    private var lastInteractiveRemember = false
        ...
        try {
            ...
            if (!pubkeysExhausted &&
                ...
                if (connection?.authenticateWithKeyboardInteractive(currentHost.username, this) == true) {
                    // Use the captured flag (lastInteractiveRemember) because
                    // PromptManager.rememberPasswordRequested may have been
                    // overwritten by a later non-password challenge prompt.
                    lastInteractivePassword?.let { pw ->
                        savePasswordIfRequested(currentHost, pw, lastInteractiveRemember)
                        lastInteractivePassword = null
                        lastInteractiveRemember = false
                    }
                    finishConnection()
                } else {
        ...
        try {
            ...
            when (credential.type) {
                ...
                CredentialType.SSH_KEY -> {
                    ...
                    try {
                        val pair = decodeStoredPrivateKey(privateKeyBytes, passphrase, credential.publicKey)
                        if (pair == null) {
    ...
    /**
     * Decode a stored private key from PKCS8 DER bytes (as generated by KeyPairGenerator.encoded)
     * or fall back to PEM decoding for keys imported from files.
     *
     * @param storedPublicKey The public key string from the credential record (format: "Algorithm <base64-SPKI> label")
     */
    private fun decodeStoredPrivateKey(privateKeyBytes: ByteArray, passphrase: String?, storedPublicKey: String? = null): KeyPair? {
        // First try PKCS8 DER (format used when key is generated in-app via KeyPairGenerator)
        try {
            val algo = detectPkcs8Algorithm(privateKeyBytes)
            val spec = java.security.spec.PKCS8EncodedKeySpec(privateKeyBytes)
            val kf = java.security.KeyFactory.getInstance(algo)
            val privateKey = kf.generatePrivate(spec)
            val publicKey = storedPublicKey?.let { parseStoredPublicKey(it) }
                ?: derivePublicKeyFromPrivate(privateKey)
                ?: return null
            return KeyPair(publicKey, privateKey)
        } catch (e: Exception) {
            Timber.e(e, "decodeStoredPrivateKey: PKCS8 DER path failed")
        }

        // Fall back to PEM decoding (for imported keys)
        return try {
            val pemString = String(privateKeyBytes, StandardCharsets.UTF_8)
            PEMDecoder.decode(pemString.toCharArray(), passphrase)
        } catch (e: Exception) {
            Timber.e(e, "decodeStoredPrivateKey: PEM fallback also failed")
            null
        }
    }

    private fun parseStoredPublicKey(storedPublicKey: String): java.security.PublicKey? = try {
        val parts = storedPublicKey.trim().split(" ")
        if (parts.size < 2) return null
        val algorithm = parts[0]
        val spkiDer = android.util.Base64.decode(parts[1], android.util.Base64.DEFAULT)
        val kf = java.security.KeyFactory.getInstance(algorithm)
        kf.generatePublic(java.security.spec.X509EncodedKeySpec(spkiDer))
    } catch (e: Exception) {
        Timber.e(e, "parseStoredPublicKey failed")
        null
    }

    private fun detectPkcs8Algorithm(der: ByteArray): String {
        // Ed25519 OID: 1.3.101.112 → bytes 06 03 2B 65 70
        // Scan offsets 6-9 to handle minor PKCS8 wrapper variations across Android versions
        for (offset in listOf(8, 7, 6, 9)) {
            if (der.size > offset + 4 &&
                der[offset] == 0x06.toByte() && der[offset + 1] == 0x03.toByte() &&
                der[offset + 2] == 0x2B.toByte() && der[offset + 3] == 0x65.toByte() &&
                der[offset + 4] == 0x70.toByte()) {
                return "Ed25519"
            }
        }
        return "RSA"
    ...
    private fun derivePublicKeyFromPrivate(privateKey: java.security.PrivateKey): java.security.PublicKey? = try {
        if (privateKey is java.security.interfaces.RSAPrivateCrtKey) {
            val spec = java.security.spec.RSAPublicKeySpec(privateKey.modulus, privateKey.publicExponent)
     ...
     */
    private fun savePasswordIfRequested(
        currentHost: Host,
        password: String,
        rememberOverride: Boolean = false
    ) {
        val shouldSave = rememberOverride ||
            bridge?.promptManager?.rememberPasswordRequested == true
        if (!shouldSave) {
            Timber.d("savePasswordIfRequested: skipped (rememberPasswordRequested=false)")
            return
        }

        // If the host already has a linked credential that still works, skip.
        // But if the credential failed during this session, allow re-saving.
        if (currentHost.credentialId != null && !credentialFailed) {
            Timber.d("savePasswordIfRequested: skipped (credentialId already set and not failed)")
            return
        }
        ...
        runBlocking {
            try {
                ...
                // Update BOTH transport and bridge in-memory host references
                host = updatedHost
                bridge?.host = updatedHost
                credentialFailed = false
                Timber.d("savePasswordIfRequested: saved credential %d for host %d", credentialId, currentHost.id)
    ...
    override fun connect() {
        ...
        try {
            ...
            var tries = 0
            while (connected && bridge?.isDisconnected != true &&
                connection?.isAuthenticationComplete != true && tries++ < AUTH_TRIES
            ) {
                authenticate()
    ...
    override fun write(buffer: ByteArray) {
        ...
        if (out == null) {
            Timber.w(
                "SSH write dropped: stdin not ready (%d bytes, connected=%s sessionOpen=%s)",
                buffer.size,
                connected,
                sessionOpen
            )
    ...
    override fun write(c: Int) {
        val out = stdin
        if (out == null) {
            Timber.w("SSH write dropped: stdin not ready (int char, sessionOpen=%s)", sessionOpen)
            return
        }
        out.write(c)
    }
    ...
    /**
     * Sends an SSH transport-level ping so NATs and firewalls do not drop idle TCP sessions.
     * Safe to call periodically from a background coroutine when the user enabled keep-alive.
     */
    fun sendConnectionKeepalive() {
        try {
            connection?.ping()
        } catch (e: IOException) {
            Timber.d(e, "SSH keepalive ping failed")
        }
    }
    ...
    @Throws(IOException::class)
    override fun uploadTerminalAttachment(bytes: ByteArray, filename: String): String? {
        val conn = connection ?: return null
        if (!connected || !authenticated) return null

        val safeName = filename
            .replace(Regex("""[^A-Za-z0-9._-]"""), "_")
            .ifBlank { "sapienterm-image.jpg" }

        val sftp = SFTPv3Client(conn)
        return try {
            val home = sftp.canonicalPath(".").trimEnd('/')
            val root = "$home/.sapienterm"
            val uploadDir = "$root/uploads"
            try {
                sftp.mkdir(root, 0x1C0) // 0700
            } catch (_: IOException) {
                // Already exists, or the upload below will report the real failure.
            }
            try {
                sftp.mkdir(uploadDir, 0x1C0) // 0700
            } catch (_: IOException) {
                // Already exists, or the upload below will report the real failure.
            }

            val remotePath = "$uploadDir/$safeName"
            val handle = sftp.createFileTruncate(remotePath)
            try {
                var offset = 0
                while (offset < bytes.size) {
                    val count = minOf(32 * 1024, bytes.size - offset)
                    sftp.write(handle, offset.toLong(), bytes, offset, count)
                    offset += count
                }
            } finally {
    ...
    override fun replyToChallenge(
        ...
        val responses = Array(numPrompts) { i ->
            ...
            if (isPassword && response.isNotEmpty()) {
                ...
                // Capture the "remember" flag now, before a subsequent
                // non-password prompt can reset it.
                lastInteractiveRemember =
                    bridge?.promptManager?.rememberPasswordRequested == true
            }
            response
        }
