...
interface HostDao {
    ...
    /** Toggle the pinned flag on a single host. */
    @Query("UPDATE hosts SET pinned = :pinned WHERE id = :id")
    suspend fun setPinned(id: Long, pinned: Boolean)

    /** Observe pinned hosts (alphabetical). */
    @Query("SELECT * FROM hosts WHERE pinned = 1 ORDER BY nickname COLLATE NOCASE")
    fun observePinned(): Flow<List<Host>>

    /** Observe recent hosts by last_connect desc, limited to `limit`. Hosts never connected are excluded. */
    @Query("SELECT * FROM hosts WHERE last_connect > 0 ORDER BY last_connect DESC LIMIT :limit")
    fun observeRecent(limit: Int): Flow<List<Host>>
}
