...
data class Host(
    ...
    /**
     * Fixed terminal columns. When set (along with [fixedRows]), the terminal
     * uses this exact size and never resizes — useful for tmux shared sessions.
     * null = auto-size to fit the screen.
     */
    @ColumnInfo(name = "fixed_cols")
    val fixedCols: Int? = null,

    /**
     * Fixed terminal rows. See [fixedCols].
     */
    @ColumnInfo(name = "fixed_rows")
    val fixedRows: Int? = null,

    /**
     * Optional override for the session bottom bar layout.
     * `null` means use the app default from [com.logicalsapien.sapienterm.util.PreferenceConstants.TERMINAL_BOTTOM_BAR_PRESET].
     * Otherwise same stored values as that preference: `default`, `tmux`, or `custom:…`.
     */
    @ColumnInfo(name = "bottom_bar_preset_override")
    val bottomBarPresetOverride: String? = null,

    /**
     * Optional override for software keyboard on session start (`auto`, `on`, `off`).
     * `null` uses app defaults for the effective bottom bar preset.
     */
    @ColumnInfo(name = "session_keyboard_override")
    val sessionKeyboardOverride: String? = null,

    /**
     * Whether this host is pinned. Pinned hosts surface in the Search tab's
     * empty state alongside recent connections. Local-only flag, not exported.
     */
