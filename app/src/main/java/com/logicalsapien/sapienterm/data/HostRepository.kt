...
class HostRepository @Inject constructor(
    ...
    /** Set the pinned flag on a host. */
    suspend fun setPinned(hostId: Long, pinned: Boolean) {
        hostDao.setPinned(hostId, pinned)
    }

    fun observePinnedHosts(): kotlinx.coroutines.flow.Flow<List<Host>> = hostDao.observePinned()

    fun observeRecentHosts(limit: Int = 5): kotlinx.coroutines.flow.Flow<List<Host>> = hostDao.observeRecent(limit)
    ...
    suspend fun saveKnownHost(
        ...
        if (host.id <= 0L) {
            // Temporary URI hosts are not present in the hosts table, so there is
            // no valid parent row for the known_hosts foreign key. The current
            // connection can still continue after explicit user acceptance.
            return
        }
