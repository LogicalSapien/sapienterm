...
import com.logicalsapien.sapienterm.util.ProviderLoader
import com.logicalsapien.sapienterm.util.ProviderLoaderListener
...
class TerminalManager :
    ...
    @Inject
    internal lateinit var securePasswordStorage: com.logicalsapien.sapienterm.util.SecurePasswordStorage

    @Inject
    internal lateinit var credentialRepository: com.logicalsapien.sapienterm.data.CredentialRepository
    ...
    /**
     * Per-bridge detected prompts. Maps host ID to the detected prompt (or null).
     * Scanned periodically for ALL active bridges so background tabs are monitored.
     */
    private val _bridgePrompts = MutableStateFlow<Map<Long, DetectedPrompt>>(emptyMap())
    val bridgePrompts: StateFlow<Map<Long, DetectedPrompt>> = _bridgePrompts.asStateFlow()

    /**
     * The host ID of the bridge currently visible in the UI.
     * Used to decide whether to fire a notification (only for non-visible bridges).
     */
    @Volatile
    var visibleBridgeHostId: Long = -1L

    /** The currently visible bridge (if any), derived from [visibleBridgeHostId]. */
    val visibleBridge: TerminalBridge?
        get() = synchronized(_bridges) {
            _bridges.find { it.host.id == visibleBridgeHostId }
        }

    private var promptScanJob: Job? = null

    /**
     * Tracks which host IDs have already triggered a prompt notification
     * to avoid spamming the same notification repeatedly.
     */
    private val notifiedPromptHosts = mutableSetOf<Long>()
    ...
    private fun openConnection(host: Host): TerminalBridge {
        ...
        // Apply light theme override if the app is currently in light mode
        ...
        val isLight = when (themeMode) {
            ...
            "dark" -> false
                ...
                uiModeFlags != android.content.res.Configuration.UI_MODE_NIGHT_YES
            }
        }
        if (isLight) {
            bridge.applyThemeOverride(true)
        }
    ...
    fun getScrollback(): Int {
        var scrollback = 8192
        try {
            scrollback = Integer.parseInt(prefs.getString(PreferenceConstants.SCROLLBACK, "8192")!!)
        } catch (_: Exception) {
        }
        return scrollback
    }
    ...
    private fun startPromptScanning() {
        promptScanJob?.cancel()
        promptScanJob = scope.launch {
            while (true) {
                delay(PROMPT_SCAN_INTERVAL_MS)
                scanAllBridgesForPrompts()
            }
        }
    }

    private fun scanAllBridgesForPrompts() {
        val bridges: List<TerminalBridge>
        synchronized(_bridges) {
            bridges = ArrayList(_bridges)
        }

        val newPrompts = mutableMapOf<Long, DetectedPrompt>()
        val previousPrompts = _bridgePrompts.value

        for (bridge in bridges) {
            if (!bridge.isSessionOpen) continue
            val hostId = bridge.host.id
            val lines = bridge.getRecentOutputLines()
            val detected = CliPromptDetector.detect(lines)
            if (detected != null) {
                newPrompts[hostId] = detected

                val isVisible = hostId == visibleBridgeHostId && isUiBound
                val wasAlreadyNotified = hostId in notifiedPromptHosts
                val isNewPrompt = previousPrompts[hostId] != detected

                Timber.d("Prompt detected: type=${detected.type}, host=${bridge.host.nickname}, visible=$isVisible, uiBound=$isUiBound, alreadyNotified=$wasAlreadyNotified, isNew=$isNewPrompt")

                if (!isVisible && isNewPrompt && !wasAlreadyNotified) {
                    Timber.i("Sending prompt notification for host=${bridge.host.nickname}, type=${detected.type}")
                    notifiedPromptHosts.add(hostId)
                    connectionNotifier.showPromptNotification(
                ...
                if (hostId in notifiedPromptHosts) {
                    notifiedPromptHosts.remove(hostId)
                    connectionNotifier.cancelPromptNotification(this@TerminalManager, hostId)
                }
            }
        }

        _bridgePrompts.value = newPrompts
    }
    ...
    /**
     * Apply a light/dark theme override to all active terminal bridges.
     */
    fun applyThemeToAllBridges(isLight: Boolean) {
        synchronized(_bridges) {
            _bridges.forEach { it.applyThemeOverride(isLight) }
        }
    }
    ...
    companion object {
        ...
        private const val PROMPT_SCAN_INTERVAL_MS = 2000L
    }
}
