...
class ConnectivityMonitor(
    ...
    private val networkCallback = object : ConnectivityManager.NetworkCallback() {
        ...
        override fun onLost(network: Network) {
            ...
            // onLost is for one Network object; the device may still have internet via another
            // transport (e.g. WiFi ↔ cellular handoff, AP roam). activeNetwork can lag briefly;
            // scan all networks for a validated one before declaring offline.
            val validated = findFirstValidatedNetwork()
            if (validated != null) {
                Timber.i("Validated network still available after loss ($validated) — refreshing state, no disconnect")
                updateNetworkInfo(validated)
                return
            }
    ...
    @Suppress("DEPRECATION") // allNetworks: no drop-in until we track networks via callbacks
    private fun findFirstValidatedNetwork(): Network? {
        connectivityManager.activeNetwork?.let { if (hasValidatedInternet(it)) return it }
        for (network in connectivityManager.allNetworks) {
            if (hasValidatedInternet(network)) return network
        }
        return null
    }
