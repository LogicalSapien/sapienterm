...
import com.logicalsapien.sapienterm.ui.MainActivity
...
/**
 * Manages notifications for background SSH / terminal sessions.
 *
 * @author Kenny Root
 *
...
class ConnectionNotifier @Inject constructor() {
    ...
    private fun createNotificationChannel(context: Context, id: String) {
        val isPrompt = id == PROMPT_CHANNEL
        val nc = NotificationChannel(
            id,
            if (isPrompt) "Terminal Prompts" else context.getString(R.string.app_name),
            if (isPrompt) NotificationManager.IMPORTANCE_HIGH else NotificationManager.IMPORTANCE_DEFAULT
        )
        if (isPrompt) {
            nc.description = "Alerts when a terminal session is waiting for input"
        }
    ...
    fun showPromptNotification(context: Service, host: Host, promptType: PromptType) {
        val res = context.resources
        val name = host.nickname.ifBlank { host.hostname }
        val contentText = when (promptType) {
            PromptType.PASSWORD -> res.getString(R.string.notification_prompt_text_password, name)
            PromptType.QUESTION -> res.getString(R.string.notification_prompt_text_question, name)
            PromptType.YES_NO -> res.getString(R.string.notification_prompt_text_yesno, name)
            else -> res.getString(R.string.notification_prompt_text_generic, name)
        }

        val builder = newNotificationBuilder(context, PROMPT_CHANNEL)
        ...
        val contentIntent = PendingIntent.getActivity(
            context,
            PROMPT_NOTIFICATION_BASE + host.id.toInt(),
            notificationIntent,
            pendingIntentFlags
        )

        builder.setContentTitle(res.getString(R.string.notification_prompt_title, name))
            .setContentText(contentText)
            .setContentIntent(contentIntent)
            .setAutoCancel(true)
    ...
    fun cancelPromptNotification(context: Context, hostId: Long) {
        getNotificationManager(context).cancel(PROMPT_NOTIFICATION_BASE + hostId.toInt())
    }
    ...
    companion object {
        ...
        private const val PROMPT_NOTIFICATION_BASE = 1000
        private const val NOTIFICATION_CHANNEL = "my_connectbot_channel"
        private const val PROMPT_CHANNEL = "sapienterm_prompt_channel"
    }
}
