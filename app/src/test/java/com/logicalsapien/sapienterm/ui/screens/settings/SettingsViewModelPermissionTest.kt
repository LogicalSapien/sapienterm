...
class SettingsViewModelPermissionTest {
    ...
    fun setUp() = runTest {
        ...
        exportManager = mock()
        importManager = mock()
        autoBackupManager = mock()
        customBottomBarLayoutStore = mock()
        whenever(customBottomBarLayoutStore.loadAll()).thenReturn(emptyList())
        whenever(autoBackupManager.isEnabled).thenReturn(false)
        whenever(autoBackupManager.retentionCount).thenReturn(AutoBackupManager.DEFAULT_RETENTION)
        whenever(autoBackupManager.lastBackupTime).thenReturn(0L)
        whenever(autoBackupManager.listBackups()).thenReturn(emptyList())
        ...
        // Avoid session-keyboard migration write on every init (extra prefs.apply()) so connPersist tests only assert their own edits.
        whenever(prefs.contains(PreferenceConstants.TERMINAL_SESSION_KEYBOARD)).thenReturn(true)
